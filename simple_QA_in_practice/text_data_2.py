...
class TextData:
    ...
    def create_corpus(self, data_path, sorted_list_path, dialog_path, vocab_size):
        ...
        if(len(conversation)<max_len):
            max_len = len(conversation)
        for index in tqdm(range(1,max_len)):
            inputWords = self.extract_text(nltk.word_tokenize(conversation.iloc[index]['Context'].decode('utf8','ignore')))
            targetWords = self.extract_text(nltk.word_tokenize(conversation.iloc[index]['Ground Truth Utterance'].decode('utf8','ignore')))

            if inputWords and targetWords:
                if len(inputWords)>self.max_context_len:
                    inputWords = inputWords[-self.max_context_len:] # inversed...?
    ...
    def remove_tag(self, data_path):
        ...
        df['Ground Truth Utterance'] = df['Ground Truth Utterance'].str.replace('__eou__','') # why "Context" not "Ground Truth Utterance"?
        df['Ground Truth Utterance'] = df['Ground Truth Utterance'].str.replace('__eot__','')
        return df
...
sorted_list_path = '/Users/hanzhichao/Documents/github/Conversational-Agents'
t = TextData('ubuntu_valid_samples.pkl', rootDir+'data/ubuntu/valid.csv', sorted_list_path, rootDir+'data/ubuntu/dialogs/', 50000, 0)
b = t.get_batches(50)
